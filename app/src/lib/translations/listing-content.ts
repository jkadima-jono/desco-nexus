...
const fr = {
  ...
  "comicordia-agri": {
    ...
    sector: "Agriculture", country: "RDC", instrument: "Structure de capital propre au projet non encore définie", stage: "Informations d’exploitation fournies", irr: "Aucune projection publique de rendement n’est publiée",
...
const ptCopy: Record<string, Pick<ListingText, "summary" | "highlights" | "whyMatch">> = {
  ...
  "comicordia-agri": { summary: "Apresentamos uma plataforma agrícola no centro da RDC que organizaria pequenos produtores através de crédito de insumos, mecanização partilhada, armazenamento e transformação de milho, mandioca e soja. Antes de avançar, exigimos validar terras, participação dos agricultores, rendimentos, compra e resultados declarados.", highlights: ["Meta: participação material de pequenos produtores e expansão da área cultivada, a definir e comprovar no piloto", "Meta: melhorar a cobertura das aldeias e o rendimento familiar, com linha de base e método ainda necessários", "O emprego será publicado apenas depois de definida a linha de base e o método de medição"], whyMatch: "Oportunidade agrícola que exige validar alcance, acesso à terra, compra, salvaguardas e impacto declarado." },
...
const zhCopy: Record<string, Pick<ListingText, "summary" | "highlights" | "whyMatch">> = {
  ...
  "comicordia-agri": { summary: "我们展示一项位于刚果民主共和国中部的农业平台机会，计划通过投入品信贷、共享机械化、收后仓储及玉米、木薯和大豆加工组织小农网络。推进前，我们要求核实土地、农户参与、产量、承购及所述成果。", highlights: ["目标：扩大可核实的小农参与和耕作面积，具体范围将在试点阶段确定", "目标：改善村庄覆盖和家庭收入，仍须建立基线及测量方法", "就业数据将在基线及测量方法确定后发布"], whyMatch: "需核验农户覆盖、土地使用权、承购、保障措施及所述影响的农业项目。" },
...
const es = Object.fromEntries(Object.entries(fr).map(([id, x]) => [id, {
  ...
  instrument: ({
    ...
    "comicordia-agri": "Estructura de capital específica del proyecto aún no definida",
...
const pt = Object.fromEntries(Object.entries(fr).map(([id, x]) => [id, {
  ...
  instrument: ({
    ...
    "comicordia-agri": "Estrutura de capital específica do projeto ainda não definida",
...
const evidenceLabels: Record<Exclude<Locale, "en">, Record<string, string>> = {
  // Project-specific fields use the same locale coverage as the common
  // disclosure model so switching language never exposes English headings.
...
Object.assign(evidenceLabels.fr, { "Capital estimate": "Estimation du coût", "Scope and cost baseline": "Périmètre et base de coûts", "Commercial and demand": "Commercial et demande", "Delivery and governance": "Exécution et gouvernance" });
Object.assign(evidenceLabels.es, { "Capital estimate": "Estimación de costes", "Scope and cost baseline": "Alcance y base de costes", "Commercial and demand": "Comercial y demanda", "Delivery and governance": "Ejecución y gobernanza" });
Object.assign(evidenceLabels.pt, { "Capital estimate": "Estimativa de custos", "Scope and cost baseline": "Âmbito e base de custos", "Commercial and demand": "Comercial e procura", "Delivery and governance": "Execução e governação" });
Object.assign(evidenceLabels.zh, { "Capital estimate": "成本估算", "Scope and cost baseline": "范围与成本基准", "Commercial and demand": "商业与需求", "Delivery and governance": "交付与治理" });
