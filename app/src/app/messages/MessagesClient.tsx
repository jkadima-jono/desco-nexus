    ...
    <div className="h-[calc(100vh-4rem)] lg:h-screen flex min-w-0">
      ...
      <div className={(!mobileThreadOpen ? "hidden lg:flex " : "flex ") + "min-w-0 flex-1 flex-col bg-mist"}>
        <div className="bg-white border-b border-charcoal/10 px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-3">
          <Button type="button" onClick={() => setMobileThreadOpen(false)} className="lg:hidden min-w-10 min-h-10  border border-charcoal/10" aria-label={t("messages.backToConversations")}>←</Button>
        ...
        <div className="flex-1 overflow-y-auto px-3 sm:px-6 py-5 space-y-3">
          {active.messages.map((m, i) =>
            m.from === "system" ? (
              <div key={i} className="text-center">
                <span className="inline-block text-[11px] bg-charcoal/5 text-wgray px-3 py-1 ">
                  ◆ {m.text}
                </span>
              </div>
            ) : (
          ...
          {sent.map((m, i) => (
            <div key={"x" + i} className="flex justify-end">
              <div className="max-w-md px-4 py-2.5   text-sm bg-charcoal text-white">
                {m.text}
        ...
        {aiOpen && (
          <div className="mx-3 sm:mx-4 mb-2  border border-gold/40 bg-gold-soft p-3 text-sm">
            <div className="font-display font-bold mb-1">AI draft for {active.name}</div>
            {aiBusy ? (
              <p className="text-xs text-wgray">Generating…</p>
            ) : aiDraft ? (
              <>
                <p className="text-xs whitespace-pre-wrap bg-white  p-2 mb-2">{aiDraft.text}</p>
            ...
            <div className="flex gap-2">
              <Button
                ...
                className="bg-charcoal text-white px-3 py-2  text-xs font-bold disabled:opacity-50"
              >
                Insert draft
              </Button>
              <Button type="button" onClick={() => { setAiOpen(false); setAiDraft(null); }} className="px-3 py-2  text-xs font-bold">Cancel</Button>
            </div>
          </div>
        )}
        <div className="p-3 sm:p-4 bg-white border-t border-charcoal/10 flex items-end gap-2 sm:gap-3">
          <Button
            ...
            className="min-w-11 min-h-11  border border-charcoal/10 text-gold font-bold"
          ...
          <textarea
            ...
            className="min-w-0 flex-1 resize-none bg-mist  px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-gold"
          />
          <Button
            onClick={send}
            disabled={!draft.trim() || sending}
            className="min-h-11 bg-gold text-ink font-display font-bold text-sm px-4 sm:px-5  hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed"
          >
