      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className=" bg-white p-5 ">
          ...
          <div className="mt-4 space-y-2">
            {byEvent.map((row) => <div key={row.event} className="flex justify-between  bg-mist px-3 py-2 text-sm"><span>{row.event.replaceAll("_", " ")}</span><strong>{row._count}</strong></div>)}
        ...
        <section className=" bg-white p-5 ">
          ...
          <div className="mt-4 space-y-2">
            {byPath.slice(0, 20).map((row) => <div key={row.path} className="flex gap-3  bg-mist px-3 py-2 text-sm"><span className="min-w-0 flex-1 truncate">{row.path}</span><strong>{row._count}</strong></div>)}
    ...
    <div>
      ...
      <ol className="mt-3 space-y-2">
        {stages.map(([event, label], index) => (
          <li key={event} className="flex items-center gap-3  bg-mist px-3 py-2 text-sm">
            <span className="grid h-6 w-6 shrink-0 place-items-center  bg-ink text-xs font-bold text-white">{index + 1}</span>
