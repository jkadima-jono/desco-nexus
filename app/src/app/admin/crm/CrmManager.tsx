...
const input = "w-full  border border-black/10 bg-white px-3 py-2.5 text-sm";
const button = "w-full  bg-ink px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50";
...
export default function CrmManager({
  ...
  return (
    <div className="space-y-6">
      {error && <p role="alert" className=" bg-red-50 px-4 py-3 text-sm text-red-800">{error}</p>}

      <section className="grid gap-4 xl:grid-cols-4">
        <form className=" bg-white p-5 " onSubmit={async (event) => {
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className=" bg-white p-5 ">
          <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-display text-lg font-bold">Relationship directory</h2><input className=" border border-black/10 px-3 py-2 text-sm" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search relationships" aria-label="Search relationships" /></div>
          <div className="mt-4 space-y-3">{filteredContacts.length === 0 ? <p className="text-sm text-wgray">No matching relationships.</p> : filteredContacts.map((contact) => (
            <div key={contact.id} className="flex flex-wrap items-center justify-between gap-3  border border-black/10 p-3">
        ...
        <div className=" bg-white p-5 ">
          ...
          <div className="mt-4 space-y-3">{opportunities.length === 0 ? <p className="text-sm text-wgray">No opportunities recorded.</p> : opportunities.map((o) => (
            <div key={o.id} className=" border border-black/10 p-3">
        ...
        <div className=" bg-white p-5 ">
          ...
          <div className="mt-4 space-y-3">{tasks.length === 0 ? <p className="text-sm text-wgray">No follow-ups recorded.</p> : tasks.map((task) => (
            <div key={task.id} className="flex items-center justify-between gap-3  border border-black/10 p-3"><div><p className="font-bold">{task.title}</p><p className="text-sm text-wgray">{task.opportunity?.name || "General"}{task.dueAt ? ` · due ${new Date(task.dueAt).toLocaleDateString()}` : ""} · {task.priority}</p></div><select className=" border border-black/10 px-2 py-2 text-sm" value={task.status} onChange={(event) => void send("PATCH", { entity: "task", id: task.id, status: event.target.value })}><option value="open">Open</option><option value="in-progress">In progress</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></div>
          ))}</div>
        </div>
        <div className=" bg-white p-5 ">
          ...
          <div className="mt-4 space-y-3">{activities.length === 0 ? <p className="text-sm text-wgray">No activity recorded.</p> : activities.map((activity) => (
            <div key={activity.id} className=" border border-black/10 p-3"><div className="flex justify-between gap-3"><p className="font-bold">{activity.subject}</p><span className="text-sm uppercase text-wgray">{activity.type}</span></div><p className="mt-1 text-sm text-wgray">{activity.contact ? `${activity.contact.firstName} ${activity.contact.lastName}` : activity.opportunity?.name || "General"} · {new Date(activity.occurredAt).toLocaleDateString()}</p>{activity.body && <p className="mt-2 text-sm">{activity.body}</p>}</div>
          ))}</div>
        </div>
      </section>
    </div>
