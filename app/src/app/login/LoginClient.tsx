      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link href="/" aria-label="Return to DESCO Compass home" className="inline-block">
            {/* eslint-disable-next-line @next/next/no-img-element */}<img src="/brand/desco-coin.png" alt="DESCO Compass seal" className="mx-auto mb-4 h-28 w-28 rounded-full object-cover" />
          </Link>
          ...
          {demoEnabled ? (
            <>
              ...
              <div className="grid gap-3">{PERSONAS.filter((p) => p.id !== "admin" || adminEnabled).map((p) => (
                <Button key={p.id} onClick={() => enterDemo(p.id)} disabled={!!busy} className="flex min-h-11 items-center gap-3 w-full text-left px-4 py-3.5  border-2 border-charcoal/10 hover:border-gold hover:bg-gold-soft font-display font-bold text-sm transition-colors disabled:opacity-60">
                  <span aria-hidden="true" className="w-8 h-8 bg-charcoal text-gold flex items-center justify-center">{p.icon}</span>{busy === p.id ? t("login.busy") : t(p.labelKey)}
                </Button>
              ))}</div>
            ...
            <div role="status" aria-live="polite" className="text-center py-3">
              <div aria-hidden="true" className="mx-auto mb-4 grid h-12 w-12 place-items-center  bg-gold-soft text-ink">✓</div>
            ...
            <>
              ...
              <form onSubmit={requestLink} className="space-y-4">
                {mode === "signup" && <div><label htmlFor="fullName" className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-wgray">{account.fullName}</label><input id="fullName" name="fullName" autoComplete="name" required minLength={2} maxLength={150} value={fullName} onChange={(e) => setFullName(e.target.value)} className="min-h-11 w-full  bg-mist px-4 py-3 text-sm" /></div>}
                <div><label htmlFor="email" className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-wgray">{account.email}</label><input id="email" name="email" autoComplete="email" type="email" required maxLength={254} value={email} onChange={(e) => setEmail(e.target.value)} className="min-h-11 w-full  bg-mist px-4 py-3 text-sm" /></div>
                {mode === "signup" && <><div className=" border border-gold/25 bg-gold-soft p-3 text-xs leading-5 text-charcoal">{account.basicAccountNotice}</div><label className="flex items-start gap-3 text-xs leading-5 text-slate"><input type="checkbox" required checked={acceptedTerms} onChange={(e) => setAcceptedTerms(e.target.checked)} className="mt-0.5 h-5 w-5 shrink-0 accent-gold" /><span>{account.termsAcceptance} <Link href="/legal" className="font-bold text-ink underline underline-offset-2">{account.termsLink}</Link>.</span></label></>}
                {error && <div role="alert" className=" bg-brandred/10 px-3 py-2 text-xs text-brandred">{error}</div>}
            ...
            <div className="text-center"><h1 className="font-display font-bold text-xl">{account.unavailableTitle}</h1><p className="mt-3 text-sm leading-6 text-wgray">{account.unavailableBody}</p><Button href="/contact?topic=investor-access" className="button-primary mt-5 w-full">{account.contact}</Button></div>
          )}
          <Button href="/opportunities" className="button-secondary mt-5 w-full">{copy.backToOpportunities}</Button>
        </div>
